...
% Scale-Factor-Free Super-Resolution
% Author: Jiwon Kim (jiwon@alum.mit.edu)
...
exp_name = sprintf('exp_sf%s_resid%d_depth%d', mat2str(opts.sfs), opts.resid, opts.depth);
...
if ~opts.continue || ~exist(sprintf('data/trainData/%s.mat',exp_name),'file')
    imdb = generateData(opts.dataDir, opts.sfs, opts.depth, opts.pad, opts.resid);
...
function imdb = generateData(dataDir, sfs, depth, pad, diff)
