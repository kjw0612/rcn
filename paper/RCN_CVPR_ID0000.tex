...
\begin{equation}
l_1(\theta) = \sum_{d=1}^D \sum_{i=1}^N \frac{1}{2}||\mathbf{y^{(i)}}-f^{(d)}(\mathbf{x^{(i)}})||^{2},
\end{equation}
where $\theta$ denotes the parameter set and $f^{(d)}$ produces the output from the $d$-th recursion. For the final output, we have 
\begin{align}
l_2(\theta) &= \sum_{i=1}^N \frac{1}{2}||\mathbf{y^{(i)}}-f(\mathbf{x^{(i)}})||^{2} \\
&= \sum_{i=1}^N \frac{1}{2}||\mathbf{y^{(i)}}-   \sum_{d=1}^D  w_d \cdot f^{(d)}(\mathbf{x^{(i)}})||^{2}
\end{align}
...
We provide parameters used to train our final model. We use a network of \textcolor{red}{depth 20}. Training uses batches of size 64. \textcolor{red}{Momentum and weight decay parameters are set to 0.9 and $0.0001$, respectively. [TODO: explain in last part of Section 3?]}
...
\multirow{4}{*}{\graphicspath{{figs/fig1/}}\includegraphics[width=0.27\textwidth]{img053_GTbox.png}}
