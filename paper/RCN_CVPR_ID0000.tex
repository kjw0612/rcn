...
%%%%%%%%% ABSTRACT
\begin{abstract}
Very deep convolutional networks (DCN) enjoy two desirable properties: wide receptive fields and high nonlinearities. We empirically find that deeper convolutional networks tend to super-resolve images better. One drawback using DCNs is that new parameters should be introduced every layer and models tend to be huge. For the reason, training an arbitrarily-deep DCN for image super-resolution is practically infeasible. To resolve the issue, we propose a deeply-recursive convolutional network (DRCN) repeatedly applying the same $3\times3$ convolutional layer many times (up to 30). Deep recursion widens receptive fields and increase nonlinearities without introducing new parameters. Training a DRCN modeling long-range dependencies between pixels is very difficult due to vanishing/exploding gradients. Gradient clipping, common to recurrent networks, resolves the issue but the network converges extremely slow. We use two techniques to boost convergence: deep-supervision and residual-learning. With them, we find DRCN converges much faster with better accuracy during training. To our surprise, very small DRCNs perform on par with gigantic DCNs (about 30 times more parameters). Moreoever, large DRCNs greatly outperform capacity-matched DCNs. Our proposed method outperform state-of-the-art methods by a large margin (TODO:number).
%
%\pagebreak
