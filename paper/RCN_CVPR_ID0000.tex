...
%%%%%%%%% ABSTRACT
\begin{abstract}
We propose an image super-resolution method (SR) using a deeply-recursive convolutional network (DRCN). DRCN repeatedly applies the same convolutional layer. Our network utilizes very large context with one recursive layer and no pooling is used. As our method reuse the same parameters all times, our network is much smaller than the CNN-based approach with the same receptive field. We sucessfully learned a SR method using a deeply-recursive network (50 recursions). Surprisingly, very small DRCNs perform on par with gigantic DCNs (about 50 times more parameters). Moreover, large DRCNs greatly outperform capacity-matched DCNs. Our proposed method outperform state-of-the-art methods by a large margin (TODO:number).
%
%\pagebreak
...
As image details are important for image restoration problems such as super-resolution and denoising, most deep-learning approaches for such problems do not use pooling. For the task of image super-resolution (SR), receptive field corresponds to the amount of contextual information that can be exploited to infer missing high-frequency components. For example, if there exist a pattern with smoothed edges contained in a receptive field, it is plausible the pattern is recognized and edges are sharpened. As SR is an ill-posed inverse problem, collecting and analyzing more neighbor pixels can possibly give more clues on what have been lost by downsampling. 
...
Increasing depth basically introduces more parameters. Two problems can arise. First, overfitting is highly likely. To prevent, more data are required. Second, model becomes too huge to be stored and retrieved (reaching gigabytes).
...
While DRCN has good properties, learning long-range dependencies between pixels is very difficult. Networks do not easily converge. This is due to exploding/vanishing gradients and this phenomena is also observed in recurrent neural networks \cite{pascanu2013difficulty}. Note the difference that our recursive layer do not have any external input/output whereas recurrent neural networks have external input/output for all timesteps. 
...
Instead of soly relying on general techniques, we propose two domain-specific approaches to ease the difficulty of training. First, all recursions are supervised in the same manner. Our deep-supervision is different from deeply-supervised nets (TODO cite). In deeply-supervised net, layers represent different levels of abstraction (edges/parts/objects) and they are supervised in a different manner. 

Second, residual images are modeled. As LR image and HR image share the same information to a large extent, explicitly modelling the residual image which is the difference between HR and LR images is advantageous. This is particularly effective when input and output are highly correlated. With residual-learning as well as deep-supervision, we find DRCN converges very well during training. 
...
\textbf{Contributions} In summary, we propose a image super-resolution method deeply recursive in nature. It utilizes very large context compared to previous SR methods with only a single convolutional layer. We suggest two ways to make this deeply-recursive convolutional network converge very fast during training: deep-supervision and residual-learning. Our algorithm demonstrates the state-of-the-art quality in common benchmarks for SR.
